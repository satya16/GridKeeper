...
Talks to the FastAPI backend (`manager/app/`) purely over the same REST
API the old dashboard used -- polling, not a live WebSocket from the
browser's side (the manager's own worker-facing WebSocket is unrelated).
...
## Build for the manager to serve
...
`build.outDir`) -- that's what `manager/app/main.py`'s `/` route serves.
...
`server.proxy`) -- run the manager separately on port 8000 as usual, then
use the Vite dev server's own URL (it'll print one, typically
`http://localhost:5173`) instead of the manager's port while developing.
