...
function DiscoveryCard({ worker, onPaired }) {
  const [code, setCode] = useState('')
  const address = worker.addresses[0] || '?'
  const backends = worker.backends.length ? ` — ${worker.backends.join(', ')}` : ''

  const handlePair = async (e) => {
    e.preventDefault()
    try {
      const result = await api.pairDiscovered(worker.discovery_id, code)
  ...
  return (
    <Card size="small" styles={{ body: { display: 'flex', flexDirection: 'column', gap: 6 } }}>
      <span style={{ fontWeight: 600 }}>{worker.hostname}</span>
    ...
    <Card title="Discovered on your network" extra={<span className="muted">{status}</span>}>
      {workers.length ? (
        <Space wrap size="middle" align="start">
          {workers.map((w) => (
            <DiscoveryCard key={w.discovery_id} worker={w} onPaired={handlePaired} />
          ))}
        </Space>
      ) : (
