...
def test_enroll_with_valid_token(client):
    ...
    resp = client.post(
        "/api/enroll",
        json={"pairing_token": token, "name": "worker-1", "os_name": "linux", "backends": ["boinc"]},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert "worker_id" in body
    assert "bearer_token" in body
    # Enroll itself needs no admin auth -- it's how a brand-new worker
    # bootstraps trust in the first place.


def test_enroll_with_used_token_fails(client):
    token = _mint_token(client)
    client.post("/api/enroll", json={"pairing_token": token, "name": "worker-a", "os_name": "linux", "backends": []})
    resp = client.post(
        "/api/enroll", json={"pairing_token": token, "name": "worker-b", "os_name": "linux", "backends": []}
    )
...
def test_enroll_inherits_group_from_pairing_token(client):
    ...
    assert workers[0]["group"] == "Lab 1"


def test_enroll_with_token_with_no_group_leaves_worker_ungrouped(client):
    token = _mint_token(client)
...
def test_enroll_duplicate_name_fails(client):
    ...
    client.post("/api/enroll", json={"pairing_token": t1, "name": "dup-worker", "os_name": "linux", "backends": []})
    t2 = _mint_token(client)
    resp = client.post(
        "/api/enroll", json={"pairing_token": t2, "name": "dup-worker", "os_name": "linux", "backends": []}
    )
    assert resp.status_code == 400
    ...
    resp2 = client.post(
        "/api/enroll", json={"pairing_token": t2, "name": "dup-worker-2", "os_name": "linux", "backends": []}
    )
