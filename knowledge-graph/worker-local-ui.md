...
files:
  - worker/grid_worker/local_ui.py
relates_to: [worker, boinc-backend, fah-backend, scheduling, metrics]
---

Optional read-only status page for a single worker machine, at
...
what it's doing, without turning every worker into something that
demands attention by default (see [[grid_manager_school_use_case]] in
memory: bulk-enrolled lab PCs should stay unobtrusive unless an admin
opts a machine in).

Toggle with `grid-worker local-ui {enable,disable,status}` (writes
...
Data flow: `worker.py::_status_loop` (the same loop that sends status to
the manager over the WebSocket, every `poll_interval_seconds`) also
...
Same escaping discipline as [dashboard-ui](dashboard-ui.md): worker
statuses/BOINC project names are worker-reported/untrusted, run through
`html.escape()`, never interpolated raw.

**Partially verified (2026-08-18):** `render_page()` and the real
`ThreadingHTTPServer` both covered by `worker/tests/test_local_ui.py` (11
