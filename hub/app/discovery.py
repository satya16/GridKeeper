"""Browses the LAN for unpaired grid-node machines advertising themselves
over mDNS (see node/grid_node/pairing.py) and keeps an in-memory list
the dashboard can poll via GET /api/discovery. Nothing here is persisted --
once a node is paired it stops advertising and simply drops out of this
list on its own.

Like the node side of this handshake, this has not been run against a
...
against a real node before relying on it.
"""
...
class DiscoveryRegistry:
    ...
    def get(self, discovery_id: str) -> dict | None:
        return self._nodes.get(discovery_id)
