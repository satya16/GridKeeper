from grid_node import local_ui


def _base_state(**overrides) -> dict:
    state = {
        "node_name": "lab-pc-04",
        "hub_url": "wss://grid.example.com",
        "connected": True,
        "backends": {},
...
def test_render_page_includes_node_name_and_connection_state():
...
def test_render_page_escapes_node_reported_strings():
    """Backend project names / node names are node-reported and
...
def test_state_box_update_replaces_wholesale():
    ...
    assert box.snapshot["node_name"] == "lab-pc-04"  # untouched fields survive
