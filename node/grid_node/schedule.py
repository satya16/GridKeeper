...
so a BOINC-active node pushes the policy there once and lets BOINC's own
daemon enforce it -- see backends/boinc.py's apply_schedule(). Folding@home
has no equivalent, so *this* module's should_run() is polled by node.py
...
logger = logging.getLogger("grid_node.schedule")
...
class PolicyHolder:
    """Asyncio-only (not thread-safe) holder for the node's current
