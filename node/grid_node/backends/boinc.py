...
def attach_project(project_url: str, account_key: str) -> dict:
    ...
    Callers (hub/app/api/nodes.py) must not persist it in plaintext
...
def apply_schedule(policy: dict) -> dict:
    ...
    watch BOINC Hub's Activity behavior) the first time you use this.
    """
    ...
    fd, path = tempfile.mkstemp(suffix=".xml", prefix="grid-node-boinc-prefs-")
    try:
