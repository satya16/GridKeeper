...
def cmd_enroll(args: argparse.Namespace) -> None:
    active_backends = daemon.detect_backends()
    resp = _http_post_json(
        f"{args.hub.rstrip('/')}/api/enroll",
        {
    ...
    cfg.save()
    print(f"Enrolled as '{args.name}' (node id {resp['node_id']}).")
    print(f"Config written to {config_path()}.")
    print(f"Detected backends: {', '.join(active_backends) or '(none)'}")
    print("Start the node with: grid-node run")
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        cfg = pairing.wait_for_pairing(daemon.detect_backends())
        logger.info("paired as '%s' -- starting normally", cfg.name)
    asyncio.run(daemon.run(cfg))
...
def cmd_local_ui(args: argparse.Namespace) -> None:
    try:
        ...
        raise SystemExit("not yet paired -- run 'grid-node enroll' or 'grid-node run' first") from None
    ...
    if args.local_ui_action == "enable":
        ...
        print("Takes effect next time 'grid-node run' starts (or restart it now).")
...
def cmd_status(args: argparse.Namespace) -> None:
    try:
        cfg = Config.load()
        print(f"Paired as '{cfg.name}' (node id {cfg.node_id}) with hub {cfg.hub_url}.")
    ...
    if code:
        ...
        print("(only valid while 'grid-node run' is active and waiting to be paired)")
    else:
        print("Not yet paired, and no node is currently waiting for pairing.")
        print("Run 'grid-node run' to enter pairing mode, or use 'grid-node enroll' for manual/token pairing.")


def main() -> None:
    parser = argparse.ArgumentParser(prog="grid-node")
    subparsers = parser.add_subparsers(dest="command", required=True)

    enroll_parser = subparsers.add_parser("enroll", help="Register this machine with GridKeeper using a pairing token")
    enroll_parser.add_argument("--hub", required=True, help="Hub base URL, e.g. http://gridkeeper:8000")
    ...
    run_parser = subparsers.add_parser(
        "run", help="Run the node; auto-enters LAN pairing mode first if not yet paired"
    )
