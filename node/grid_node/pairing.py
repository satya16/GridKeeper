...
it, and runs a tiny local HTTP listener the hub dials directly to
...
in hub/app/discovery.py match python-zeroconf's documented API, but
...
logger = logging.getLogger("grid_node.pairing")

SERVICE_TYPE = "_grid-node._tcp.local."
...
class PairingSession:
    ...
    def complete(self, node_id: str, bearer_token: str, hub_url: str, name: str | None = None) -> bool:
        with self._lock:
            if not self._verified:
                return False
            cfg = Config(hub_url=hub_url, node_id=node_id, token=bearer_token, name=name or self.name)
            cfg.save()
            try:
...
def wait_for_pairing(active_backends: list[str]) -> Config:
    ...
    try:
        while not session.done.wait(timeout=STILL_WAITING_LOG_INTERVAL_SECONDS):
            logger.info("still unpaired -- run 'grid-node status' on this machine to see the current code")
    finally:
