"""Optional read-only local status page for a single worker machine --
off by default (Config.local_ui_enabled), since workers are meant to run
...
'grid-worker local-ui enable' to see what's running/paused right now.
...
here; pausing/resuming stays a manager-dashboard action. Binds to
127.0.0.1 only, never the LAN, so enabling it never exposes anything to
other machines.

Reuses dashboard.css's color tokens (manager/app/static/dashboard.css)
for a consistent look, inlined here since the worker has no static-file
serving of its own.
"""
...
logger = logging.getLogger("grid_worker.local_ui")
...
class StateBox:
    ...
    def __init__(self, worker_name: str, manager_url: str) -> None:
        self.snapshot: dict = {
...
def render_page(state: dict) -> str:
    worker_name = html.escape(str(state.get("worker_name", "")))
    manager_url = html.escape(str(state.get("manager_url", "")))
...
<title>{worker_name or "Grid Worker"} — status</title>
<meta http-equiv="refresh" content="10">
<style>{_STYLE}</style>
</head><body>
<header>
  <h1>{worker_name or "Grid Worker"}</h1>
  <div class="muted"><span class="dot {conn_dot}"></span>{conn_label} to {manager_url}</div>
</header>
<main>
  <div class="card">
...
def start(port: int, state_box: StateBox) -> ThreadingHTTPServer:
    ...
    thread = threading.Thread(target=server.serve_forever, name="grid-worker-local-ui", daemon=True)
    thread.start()
