...
def _ws_url(manager_url: str, worker_id: str, token: str) -> str:
    ws_base = manager_url.replace("https://", "wss://").replace("http://", "ws://")
    return f"{ws_base.rstrip('/')}/ws/worker?worker_id={worker_id}&token={token}"
...
async def _command_loop(ws, policy_holder: schedule_mod.PolicyHolder, active_backends: list[str]) -> None:
    async for raw in ws:
        try:
            ...
            logger.warning("received non-JSON frame from manager, ignoring")
            continue
        ...
        if frame_type == "command":
            ...
            logger.warning("unknown frame type from manager: %r", frame_type)
...
async def run(config: Config) -> None:
    ...
    if active_backends:
        ...
        logger.warning("neither BOINC nor FAH detected on this machine -- worker will report empty status")

    url = _ws_url(config.manager_url, config.worker_id, config.token)
    ...
    if config.local_ui_enabled:
        state_box = local_ui.StateBox(config.name, config.manager_url)
        ...
        while True:
            try:
                async with websockets.connect(url, ping_interval=20, ping_timeout=20) as ws:
                    logger.info("connected to %s as '%s'", config.manager_url, config.name)
                    backoff = MIN_BACKOFF_SECONDS
                ...
                logger.exception("unexpected error in worker loop; reconnecting in %.1fs", backoff)
                if state_box is not None:
                    state_box.update(connected=False)
